...
export const loadRepositoryData = async () => {
  try {
    ...
    const repositoriesResponse = await fetch(`https://raw.githubusercontent.com/0x0w1/repostory/refs/heads/${branch}/repositories.json`);
...
export const refreshRepositoryData = async (selectedRepos) => {
  ...
  for (const repo of selectedRepos) {
    try {
      const dataUrl = `https://raw.githubusercontent.com/0x0w1/repostory/refs/heads/${branch}/repo_data/${repo.fileName}?t=${Date.now()}`;
      const response = await fetch(dataUrl, { 
        cache: 'no-cache',
        headers: {
